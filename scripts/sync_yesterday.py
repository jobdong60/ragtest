#!/usr/bin/env python
"""
모든 Fitbit 사용자의 어제 데이터를 동기화하는 cron 스크립트
매시간 정각에 실행하여 늦게 동기화된 데이터도 수집
"""
import os
import sys
import django
...
def sync_all_users_yesterday():
    """모든 사용자의 어제 데이터를 동기화"""
    # 어제 날짜 계산
    yesterday = (datetime.now() - timedelta(days=1)).strftime('%Y-%m-%d')

    log_message(f"=== Fitbit 어제({yesterday}) 전체 데이터 동기화 시작 ===")
    ...
    for fitbit_user in fitbit_users:
        try:
            log_message(f"[{fitbit_user.fitbit_user_id}] 어제 데이터 동기화 시작...")
            ...
            # 어제 날짜 전체 데이터 동기화 (하루 전체, 시간 제한 없음)
    ...
    log_message(
        f"=== 어제 데이터 동기화 완료 - 성공: {success_count}, 실패: {fail_count} ==="
    )


if __name__ == '__main__':
    sync_all_users_yesterday()
